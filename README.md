...
「我的题单」可以手动组合多个 OJ 的题目，设置顺序、角色、备注、标签和标签可见性，并通过题单 JSON 导入或导出。粘贴受支持平台的题目链接即可自动识别平台和题目标识。「个性化训练」可以把现有题单启动为带计时的训练，也支持轻松、均衡和压力三种训练模式；目标完成比例是可调整的训练参数。用户仍在原 OJ 提交，OJ Insight 同步后会自动更新训练进度。

自动组题不会把用户已有题单当作初始候选。OJ Insight 会先从可靠的 Codeforces、AtCoder 和 QOJ 目录读取题目，结合本地已做记录筛出候选池，再导出一个 AI 组题 ZIP。把 ZIP 交给大模型后，模型只能在候选范围内编排并返回题单 JSON；导入后可先查看题单，再决定是否开始训练。
...
- **跨 OJ 题单**：本地创建、查看、排序、导入和导出固定题单。
- **虚拟训练赛**：三种训练模式、可调目标完成比例、计时和同步后自动更新 AC。
- **AI 组题包**：从可靠目录筛选候选题，导出训练画像与约束，再导入大模型生成的题单 JSON。
...
1. 打开「我的题单」，粘贴题目链接并按需设置角色和备注；平台与题目标识会自动识别。
2. 直接训练时，在「个性化训练」选择已有题单、训练模式、时长和目标完成比例。
3. 自动组题时，先选择模式与可靠目录，生成候选池并导出 AI 组题 ZIP。
4. 把 ZIP 交给大模型，将模型返回的题单 JSON 导入 OJ Insight，查看确认后开始训练。
5. 在原 OJ 提交并同步对应平台，训练进度会自动更新；训练记录可导出复盘数据或删除。
