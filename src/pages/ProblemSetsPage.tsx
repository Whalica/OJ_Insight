...
export default function ProblemSetsPage({ notify, onTrain }: { notify: (message: string) => void; onTrain: (setId: number) => void }) {
  ...
  const [luoguCookie, setLuoguCookie] = useState('');
  ...
  const previewLuogu = async () => {
    ...
    try {
      const imported = await api.previewLuoguProblemSet(luoguUrl, luoguCookie);
  ...
  return <>
    <header className="topbar"><div><small>TRAINING CENTER · PROBLEM SETS</small><h1>题单</h1><p>查看、整理和分享跨 OJ 题单。点击题单进入详情，编辑和导出使用卡片右侧操作。</p></div><div className="compact-actions"><input ref={inputRef} hidden type="file" accept="application/json,.json" onChange={(event) => void onImport(event.target.files?.[0])} /><button title="导入 OJ Insight 题单 JSON" onClick={() => inputRef.current?.click()}><FileUp size={15} /><span>导入题单</span></button><button onClick={() => { if (luoguOpen) setLuoguCookie(''); setLuoguOpen(!luoguOpen); }}><Link2 size={15} /><span>导入洛谷题单</span></button><button className="primary" onClick={beginNew}><Plus size={15} /><span>新建题单</span></button></div></header>
    {luoguOpen && <section className="panel luogu-import"><div><strong>从洛谷题单链接导入</strong><small>读取题目后先预览编辑，再保存到本地。需要登录的题单可在这里临时填写 Cookie；仅用于本次读取，不保存。</small></div><div className="luogu-import-fields"><input aria-label="洛谷题单链接" value={luoguUrl} onChange={(event) => setLuoguUrl(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void previewLuogu(); }} placeholder="https://www.luogu.com.cn/training/12345" /><input aria-label="洛谷题单 Cookie（可选）" type="password" value={luoguCookie} onChange={(event) => setLuoguCookie(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void previewLuogu(); }} placeholder="洛谷 Cookie（可选，仅本次使用）" autoComplete="off" /><button className="primary" disabled={!luoguUrl.trim() || luoguLoading} onClick={() => void previewLuogu()}>{luoguLoading ? '读取中…' : '读取题目'}</button></div></section>}
