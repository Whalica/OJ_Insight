...
export default function ProblemSetsPage({ notify, onTrain }: { notify: (message: string) => void; onTrain: (setId: number) => void }) {
  ...
  const addLinks = async () => {
    ...
    const entries = parsed.flatMap((problem, index) => problem ? [{ ...emptyProblem(), position: draft.problems.length + index, problem: { ...emptyProblem().problem, ...problem } }] : []);
    let added = 0;
    let failure = '';
    if (entries.length) {
      notify(`正在识别 ${entries.length} 道题的标题与标签…`);
      const lookedUp = await Promise.allSettled(entries.map((entry) => api.lookupProblemMetadata(entry.problem)));
      const enriched = entries.flatMap((entry, index) => { const result = lookedUp[index]; return result.status === 'fulfilled' ? [{ ...entry, problem: result.value }] : []; });
      setDraft((current) => ({ ...current, problems: [...current.problems, ...enriched] }));
      added = enriched.length;
      const failedLinks = lines.filter((_, index) => parsed[index] && lookedUp[parsed.slice(0, index + 1).filter(Boolean).length - 1]?.status === 'rejected');
      invalid.push(...failedLinks);
      const firstFailure = lookedUp.find((result) => result.status === 'rejected');
      if (firstFailure?.status === 'rejected') failure = `；标题查询失败：${String(firstFailure.reason)}`;
    }
    setUrlInput(invalid.join('\n'));
    notify(`已添加 ${added} 道题${invalid.length ? `；${invalid.length} 条链接保留供重试` : ''}${failure}`);
  };
  const submit = async () => {
    if (saving) return;
    setSaving(true);
    try {
      const problems = await Promise.all(draft.problems.map(async (entry, position) => {
        const parsed = parseProblemUrl(entry.problem.url);
        const unresolved = parsed && (!entry.problem.name.trim() || entry.problem.name === parsed.name);
        return { ...entry, position, problem: unresolved ? await api.lookupProblemMetadata(entry.problem) : entry.problem };
      }));
      const saved = await saveSet({ ...draft, problems });
      setSelectedId(saved.id); setEditing(false); notify('题单已保存');
    } catch (error) { notify(`保存前未能识别题目标题：${String(error)}。请重试或手动填写显示名称。`); }
  ...
  const removeSet = async (set: ProblemSet) => { if (!confirm(`删除题单“${set.title}”？训练记录和同步数据不会被删除。`)) return; try { await deleteSet(set.id); setSelectedId(null); setEditing(false); notify('题单已删除'); } catch (error) { notify(String(error)); } };
