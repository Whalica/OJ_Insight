...
export default function ContestsPage({ notify, onOpenVp }: { notify: (message: string) => void; onOpenVp: () => void }) {
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const problems = await Promise.all(draft.problems.map(async (entry) => {
        ...
        return { ...entry, problem: unresolved ? await api.lookupProblemMetadata(entry.problem) : entry.problem };
      }));
      const result = await api.saveContest({ ...draft, problems });
      setDraft(null); await reload(); notify(`比赛“${result.title}”已保存`);
    } catch (error) { notify(`保存前未能识别题目标题：${String(error)}。请重试或手动填写名称。`); }
  ...
  const addLinks = async () => {
    ...
    const results = await Promise.allSettled(entries.map((entry) => api.lookupProblemMetadata(entry.problem)));
    const enriched = entries.flatMap((entry, index) => { const result = results[index]; return result.status === 'fulfilled' ? [{ ...entry, problem: result.value }] : []; });
    setDraft((current) => current ? { ...current, problems: [...current.problems, ...enriched] } : current);
    setLinks(lines.filter((_, index) => !parsed[index] || results[parsed.slice(0, index + 1).filter(Boolean).length - 1]?.status === 'rejected').join('\n'));
    const firstFailure = results.find((result) => result.status === 'rejected');
    notify(`已添加 ${enriched.length} 道题${firstFailure?.status === 'rejected' ? `；标题查询失败：${String(firstFailure.reason)}。链接已保留供重试` : ''}`);
  };
  ...
  const remove = async (contest: Contest) => { if (!confirm(`确定删除比赛“${contest.title}”？已开始的 VP 记录保留。`)) return; try { await api.deleteContest(contest.id); await reload(); notify('比赛已删除'); } catch (error) { notify(String(error)); } };
