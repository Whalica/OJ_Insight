...
function greeting() {
  ...
  if (hour < 18) return { title: '傍晚好', message: '今天的轨迹已经在这里了，看看还想补上哪一块。' };
  return { title: '晚上好', message: '复盘今天的提交，比单纯追求题数更接近真正的进步。' };
}

export default function DashboardPage(props: Props) {
  const { platform, platformAccounts, accountFilter, setAccountFilter, sourceFilter, setSourceFilter, timeScope, setTimeScope, range, metric, setMetric, snapshot, loading, syncing, syncTip, syncProgress, onSync, onDay, onPlatform } = props;
  const welcome = greeting(); const title = platform ? PLATFORM_META[platform].name : welcome.title;
  const years = Array.from({ length: currentYear() - 2009 }, (_, index) => currentYear() - index);
  const label = timeScope === 'until' ? '至今' : String(timeScope);
  const move = (delta: number) => { if (timeScope !== 'until') setTimeScope(Math.min(currentYear(), Math.max(2010, timeScope + delta))); };
  return <>
    <header className="topbar dashboard-head"><div><small>{platform ? `${PLATFORM_META[platform].short} · PLATFORM` : today()}</small><h1>{title}</h1><p>{platform ? `${PLATFORM_META[platform].name} 的活动砖、难度足迹和逐题记录。` : welcome.message}</p></div><button className="primary sync-button" onClick={onSync} disabled={!!syncing}><RefreshCw size={16} className={syncing ? 'spin' : ''} />{syncProgress ? `${syncProgress.done}/${syncProgress.total}` : syncing ? '同步中' : platform ? `同步 ${PLATFORM_META[platform].short}` : '同步全部'}</button></header>
    ...
    <div className="toolbar">
      ...
      <span className="toolbar-note">按北京时间统计</span>
    </div>
    ...
    {!platform && <TodayProgress rows={snapshot.platforms} onSelect={onPlatform} />}
...
function filledDifficulty(data: Snapshot['difficulty'], platform: Platform) {
  const current = data.filter((item) => item.platform === platform);
  if (platform === 'codeforces') {
    const map = new Map(current.map((item) => [Number(item.label), item]));
    return Array.from({ length: 28 }, (_, index) => { const rating = 800 + index * 100; return map.get(rating) || { platform, label: String(rating), count: 0, order: rating }; });
  }
  if (platform === 'luogu') {
    const labels = ['入门', '普及-', '普及', '普及+/提高-', '提高', '提高+/省选-', '省选/NOI-', 'NOI/NOI+/CTS']; const map = new Map(current.map((item) => [item.label, item]));
    return labels.map((label, index) => map.get(label) || { platform, label, count: 0, order: index + 1 });
  }
  return current;
}

function DifficultyProfile({ data, preferred }: { data: Snapshot['difficulty']; preferred?: Platform }) {
  const available = PLATFORM_ORDER.filter((platform) => data.some((item) => item.platform === platform));
  ...
  if (!active) return <div className="empty">当前范围没有可靠的难度数据。</div>;
  ...
  return <><div className="difficulty-tabs">{available.map((platform) => <button className={platform === active ? 'active' : ''} onClick={() => setSelected(platform)} key={platform}>{PLATFORM_META[platform].short}<span>{PLATFORM_META[platform].name}</span></button>)}</div><div className={`histogram histogram-${active}`}>{shown.map((item) => <div key={`${item.platform}-${item.label}`} title={`${item.label}：${item.count}`}><strong>{item.count}</strong><i style={{ height: `${item.count ? Math.max(8, item.count / max * 100) : 3}%`, background: difficultyColor(active, item.label, item.order) }} /><span>{item.label}</span></div>)}</div><div className="difficulty-summary"><span>有难度记录<strong>{total.toLocaleString()} 题</strong></span><span>分级方式<strong>{active === 'codeforces' ? '每 100 rating 一级' : active === 'luogu' ? 'Luogu 最新八级体系' : `${PLATFORM_META[active].name} 当前体系`}</strong></span></div></>;
}

function LeetCodeSummary({ data }: { data: Snapshot['difficulty'] }) {
  const rows = data.filter((item) => item.platform === 'leetcode'); const total = rows.reduce((sum, item) => sum + item.count, 0);
