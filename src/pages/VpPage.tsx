...
function VpCard({ item, notify, reload }: { item: TrainingMatch; notify: (message: string) => void; reload: () => Promise<void> }) {
  ...
  const state = item.status === 'waiting' ? (item.scheduledStartAt ? `赛前倒计时 ${Math.max(0, Math.ceil((item.scheduledStartAt * 1000 - now) / 1000))} 秒` : '待开始') : item.status === 'paused' ? '已暂停' : `${Math.floor(remaining / 60)}:${String(remaining % 60).padStart(2, '0')}`;
