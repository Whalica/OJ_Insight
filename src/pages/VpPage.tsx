...
function VpCard({ item, notify, reload }: { item: TrainingMatch; notify: (message: string) => void; reload: () => Promise<void> }) {
  ...
  const [countdownSeconds, setCountdownSeconds] = useState(item.countdownSeconds);
  ...
  const action = async (operation: () => Promise<unknown>, message: string) => { if (working) return; setWorking(true); try { await operation(); await reload(); notify(message); } catch (error) { notify(String(error)); } finally { setWorking(false); } };
  ...
  const remove = async () => { try { await api.deleteTrainingMatch(item.id); setConfirmDelete(false); await reload(); notify('VP 已删除'); } catch (error) { notify(String(error)); } };
  ...
  const state = item.status === 'waiting' ? (item.scheduledStartAt ? `赛前倒计时 ${Math.max(0, Math.ceil((item.scheduledStartAt * 1000 - now) / 1000))} 秒` : `待开始${item.countdownSeconds ? ` · 预设 ${item.countdownSeconds} 秒` : ''}`) : item.status === 'paused' ? '已暂停' : `${Math.floor(remaining / 60)}:${String(remaining % 60).padStart(2, '0')}`;
  return <article className={`training-match vp-session ${item.status}`}><header><button className="match-title" onClick={() => setExpanded((value) => !value)}><ChevronDown className={expanded ? 'expanded' : ''} size={16} /><div><small>{item.status === 'waiting' ? '等待开始' : item.status === 'paused' ? '暂停中' : '参赛中'}</small><h2>{item.title}</h2></div></button><strong>{state}</strong></header><div className="match-progress"><strong>{solved}/{item.problems.length} 题</strong><span>目标 {Math.round(item.targetSolveRateMin * 100)}%–{Math.round(item.targetSolveRateMax * 100)}%</span><i><b style={{ width: `${item.problems.length ? solved / item.problems.length * 100 : 0}%` }} /></i></div>
    {expanded && <><div className="vp-actions">{item.status === 'waiting' && <div className="vp-countdown-control"><span>赛前倒计时（秒）</span><input aria-label="赛前倒计时秒数" type="number" min={0} max={7200} value={countdownSeconds} disabled={!!item.scheduledStartAt} onChange={(event) => setCountdownSeconds(Number(event.target.value))} /><button disabled={working || !!item.scheduledStartAt || !Number.isInteger(countdownSeconds) || countdownSeconds < 0 || countdownSeconds > 7200} onClick={() => void action(() => api.scheduleVp(item.id, countdownSeconds), '倒计时秒数已保存；点击“开始 VP”后才开始倒计时')}>保存秒数</button></div>}{item.status === 'waiting' && <button className="primary" disabled={working || (!!item.scheduledStartAt && now < item.scheduledStartAt * 1000)} onClick={() => void action(() => api.startVp(item.id), item.countdownSeconds && !item.scheduledStartAt ? '赛前倒计时已开始' : '比赛已开始')}><Play size={15} />{item.scheduledStartAt && now < item.scheduledStartAt * 1000 ? '倒计时中' : '开始 VP'}</button>}{item.status === 'running' && <button onClick={() => void action(() => api.pauseVp(item.id), '比赛已暂停')}><Pause size={15} />暂停</button>}{item.status === 'paused' && <button className="primary" onClick={() => void action(() => api.resumeVp(item.id), '比赛继续')}><Play size={15} />继续</button>}{item.status !== 'waiting' && <button disabled={checking} onClick={() => void check()}><RefreshCw size={15} />检查提交</button>}{item.status !== 'waiting' && <button onClick={() => void action(() => api.finishTrainingMatch(item.id), '比赛已结束，进入赛后分析')}><Flag size={15} />结束比赛</button>}<button title="删除 VP" onClick={() => setConfirmDelete(true)}><Trash2 size={15} /></button></div>
      <section className="vp-general-note"><h3>整场笔记</h3><p>记录选题策略、时间分配和赛中感受；自动保存。</p><MarkdownNote value={item.generalNote} onSave={(text) => api.saveVpNote(item.id, null, text)} placeholder="例如：前 30 分钟应该先看完所有题…" /></section><div className="vp-problems">{item.problems.map((entry) => <VpProblem key={entry.position} match={item} entry={entry} submissions={submissions.filter((s) => s.platform === entry.problem.platform && s.problemKey === entry.problem.problemKey)} />)}</div>
      <p className="vp-verdict-note">提交结果：Codeforces 官方 API 与 AtCoder 可用记录会显示 AC、WA 等 verdict；其他平台依赖本地 AC 同步，缺失记录不会计作 WA。</p>
    </>}{confirmDelete && <ConfirmDialog title="删除 VP？" message={`将删除“${item.title}”及其笔记和代码绑定，无法撤销。`} onCancel={() => setConfirmDelete(false)} onConfirm={remove} />}</article>;
}
