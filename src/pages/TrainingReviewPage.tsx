...
function ReviewCard({ item, notify, reload }: { item: TrainingMatch; notify: (message: string) => void; reload: () => Promise<void> }) {
  ...
  const [codeToRemove, setCodeToRemove] = useState<number | null>(null);
  ...
  const removeCode = async () => { if (codeToRemove === null) return; try { await api.deleteVpCode(item.id, codeToRemove); setCodeFiles(await api.listVpCodeFiles(item.id)); setCodeToRemove(null); notify('代码绑定已移除'); } catch (error) { notify(String(error)); } };
  ...
  const remove = async () => { try { await api.deleteTrainingMatch(item.id); setConfirmDelete(false); await reload(); notify('记录已删除'); } catch (error) { notify(String(error)); } };
  return <article className="panel vp-review-card"><header><div><small>VP · 已结束</small><h2>{item.title}</h2><p>{item.problems.filter((p) => p.solved).length}/{item.problems.length} 题 AC · {new Date((item.endedAt || item.createdAt) * 1000).toLocaleString()}</p></div><div className="icon-actions"><button title="更新可用提交" onClick={() => void refresh()}><RefreshCw size={15} /></button><button title="导出复盘包" onClick={() => void exportPack()}><Download size={15} /></button><button title="删除记录" onClick={() => setConfirmDelete(true)}><Trash2 size={15} /></button></div></header><details><summary>查看题目、提交与笔记</summary><div className="vp-review-body"><h3>整场笔记</h3><MarkdownNote value={item.generalNote} onSave={(text) => api.saveVpNote(item.id, null, text)} placeholder="记录整场总评" />{item.problems.map((entry) => { const rows = submissions.filter((submission) => submission.platform === entry.problem.platform && submission.problemKey === entry.problem.problemKey); const attached = codeFiles.filter(([position]) => position === entry.position); return <section key={entry.position}><h4><PlatformIcon platform={entry.problem.platform} />{entry.position + 1}. {entry.problem.name || entry.problem.problemId} · {entry.solved ? 'AC' : '未 AC'}</h4><div className="vp-review-submissions">{rows.map((submission) => <a href={submission.sourceUrl} target="_blank" rel="noreferrer" key={`${submission.sourceUrl}-${submission.submittedAt}`}>{new Date(submission.submittedAt * 1000).toLocaleTimeString()} · {verdictName(submission.verdict)}</a>)}{rows.length === 0 && <small>暂无可获取的赛时提交；本地同步仅记录 AC。</small>}</div><button className="vp-code-bind" onClick={() => void bind(entry.position)}><FileCode2 size={14} />{attached.length ? '替换本地代码' : '绑定本地代码'}</button>{attached.length > 0 && <span className="vp-attached-code">已绑定：{attached.map(([, name]) => name).join('、')} <button className="danger" onClick={() => setCodeToRemove(entry.position)}>移除绑定</button></span>}<MarkdownNote value={entry.solutionNote} onSave={(text) => api.saveVpNote(item.id, entry.position, text)} placeholder="补充题目思路" /></section>; })}</div></details><footer><button className="primary" onClick={() => void exportPack()}><Download size={15} />导出给大模型的复盘包</button></footer>{confirmDelete && <ConfirmDialog title="删除复盘记录？" message={`将删除“${item.title}”及其笔记和代码绑定，无法撤销。`} onCancel={() => setConfirmDelete(false)} onConfirm={remove} />}{codeToRemove !== null && <ConfirmDialog title="移除代码绑定？" message="将移除此题绑定的本地代码副本，无法撤销。" onCancel={() => setCodeToRemove(null)} onConfirm={removeCode} />}</article>;
}
