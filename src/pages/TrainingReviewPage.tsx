...
function ReviewCard({ item, notify, reload }: { item: TrainingMatch; notify: (message: string) => void; reload: () => Promise<void> }) {
  ...
  const removeCode = async (position: number) => { if (!confirm('确定移除此题绑定的本地代码？')) return; try { await api.deleteVpCode(item.id, position); setCodeFiles(await api.listVpCodeFiles(item.id)); notify('代码绑定已移除'); } catch (error) { notify(String(error)); } };
